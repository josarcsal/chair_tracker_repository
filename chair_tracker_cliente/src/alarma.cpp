...
String obtenerProximaAlarma(HttpClient httpClient, NTPClient timeClient, String hashMac){
    ...
    for(int j = 0; j <= (maxIndex/5); j++){
        ...
        if(horaAux != ""){
            ...
            //Serial.println(strstr(diaAlarmaAux.c_str(), diaActual));
            ...
            if(diaCheck){
                if(ActualMenosAlarma < 0){ //significa que exite una hora siguiente y nos quedamos con la mas cercana(menor diferencia), tienen prioridad a las anteriores en el ciclo
                    exiteSiguiente = 1;
                    if(abs(ActualMenosAlarma) < mejorDiferencia){
                        mejorDiferencia = abs(ActualMenosAlarma);
                        res = horaAux;
                    }
